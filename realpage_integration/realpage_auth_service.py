"""
RealPage Authentication Service

Handles OAuth 2.0 authentication and connection management for RealPage's
multi-product ecosystem including OneSite, YieldStar, Velocity, and more.
"""
...
class RealPageProductType(Enum):
    """RealPage product types"""
    ONESITE = "onesite"
...
class RealPageEnvironment(Enum):
    """RealPage environments"""
...
@dataclass
class RealPageCredentials:
    """RealPage API credentials"""
    client_id: str
    client_secret: str
    redirect_uri: str
    environment: RealPageEnvironment
    scopes: List[str] = field(default_factory=list)
    webhook_secret: Optional[str] = None
    encryption_key: Optional[str] = None

@dataclass
class RealPageConnection:
    """RealPage connection details"""
    connection_id: str
    organization_id: str
    client_id: str
    product_types: List[RealPageProductType]
    access_token: str
    refresh_token: str
...
class RealPageAuthService:
    """
    RealPage Authentication Service
    
    Manages OAuth 2.0 authentication flow, token management, and secure
    credential storage for RealPage's multi-product ecosystem.
    """
    
    def __init__(self):
        # Configuration
        self.client_id = os.environ.get('REALPAGE_CLIENT_ID')
        self.client_secret = os.environ.get('REALPAGE_CLIENT_SECRET')
        self.redirect_uri = os.environ.get('REALPAGE_REDIRECT_URI')
        self.environment = RealPageEnvironment(
            os.environ.get('REALPAGE_ENVIRONMENT', 'sandbox')
        )
        self.webhook_secret = os.environ.get('REALPAGE_WEBHOOK_SECRET')
        
        # Encryption for secure credential storage
        encryption_key = os.environ.get('REALPAGE_ENCRYPTION_KEY')
        ...
        self.api_endpoints = {
            RealPageEnvironment.PRODUCTION: {
                'auth_url': 'https://oauth.realpage.com/oauth/authorize',
                'token_url': 'https://oauth.realpage.com/oauth/token',
                'revoke_url': 'https://oauth.realpage.com/oauth/revoke',
                'userinfo_url': 'https://api.realpage.com/userinfo',
                'base_api_url': 'https://api.realpage.com'
            },
            RealPageEnvironment.SANDBOX: {
                'auth_url': 'https://oauth-sandbox.realpage.com/oauth/authorize',
                'token_url': 'https://oauth-sandbox.realpage.com/oauth/token',
                'revoke_url': 'https://oauth-sandbox.realpage.com/oauth/revoke',
                'userinfo_url': 'https://api-sandbox.realpage.com/userinfo',
                'base_api_url': 'https://api-sandbox.realpage.com'
            }
        }
        
        # In-memory storage (in production, use database)
        self.active_connections: Dict[str, RealPageConnection] = {}
        ...
        self.product_scopes = {
            RealPageProductType.ONESITE: [
                'onesite:properties:read', 'onesite:properties:write',
                'onesite:units:read', 'onesite:units:write',
                'onesite:tenants:read', 'onesite:tenants:write',
                'onesite:leases:read', 'onesite:leases:write'
            ],
            RealPageProductType.YIELDSTAR: [
                'yieldstar:pricing:read', 'yieldstar:analytics:read',
                'yieldstar:recommendations:read', 'yieldstar:market_data:read'
            ],
            RealPageProductType.VELOCITY: [
                'velocity:screening:read', 'velocity:screening:write',
                'velocity:applications:read', 'velocity:applications:write'
            ],
            RealPageProductType.LEASINGDESK: [
                'leasingdesk:leads:read', 'leasingdesk:leads:write',
                'leasingdesk:marketing:read', 'leasingdesk:tours:read'
            ],
            RealPageProductType.ACTIVEBUILDING: [
                'activebuilding:operations:read', 'activebuilding:operations:write',
                'activebuilding:assets:read', 'activebuilding:residents:read'
            ],
            RealPageProductType.CROSSFIRE: [
                'crossfire:workorders:read', 'crossfire:workorders:write',
                'crossfire:maintenance:read', 'crossfire:vendors:read'
            ]
        }
        
        logger.info("RealPage Authentication Service initialized")
    ...
    def generate_authorization_url(self, organization_id: str, 
        ...
        """
        Generate OAuth 2.0 authorization URL for RealPage
        
        Args:
            organization_id: Organization identifier
            product_types: List of RealPage products to authorize
    ...
    def exchange_code_for_tokens(self, code: str, state: str) -> RealPageConnection:
        """
        ...
        try:
            ...
            logger.info(f"Successfully created RealPage connection for organization {oauth_state.organization_id}")
    ...
    def revoke_connection(self, connection_id: str) -> bool:
        """
        Revoke RealPage connection and tokens
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to revoke tokens with RealPage: {revoke_error}")
    ...
    def get_organization_connection(self, organization_id: str) -> Optional[RealPageConnection]:
    ...
    def test_connection(self, connection_id: str) -> Dict[str, Any]:
        """
        Test RealPage connection validity
    ...
    def _get_company_info(self, access_token: str) -> Dict[str, Any]:
        """Get company information from RealPage"""
        try:
            response = requests.get(
                self.api_endpoints[self.environment]['userinfo_url'],
                headers={'Authorization': f'Bearer {access_token}'},
